...
def load_settings() -> Settings:
    ...
    return Settings(
        ...
        map_tile_urls=_tile_sources(tile_style_tag(
            os.getenv("TILE_THEME", "dark").strip().lower(),
            os.getenv("TILE_PALETTE", "night").strip().lower())),
...
def darken_tile(data: bytes, palette: dict) -> bytes:
    """Перекрасить светлый тайл OSM в тёмную схему квеста."""
    ...
    flat = Image.new("L", image.size, TILE_PAPER)
    deviation = ImageChops.difference(gray, flat)
    ink = deviation.point(lambda v: min(palette["ink_max"], int(v * palette["gain"])))
    out = ImageOps.colorize(ink, black=palette["bg"], white=palette["ink"])

    # Воду и зелень узнаём по оттенку и красим отдельно, иначе они
    # превратились бы в такие же серые пятна, как дома.
    water = ImageChops.subtract(blue, red).point(lambda v: 255 if v > 14 else 0)
    park = ImageChops.subtract(green, red).point(lambda v: 255 if v > 10 else 0)
    out.paste(Image.new("RGB", image.size, palette["park"]), mask=park)
    out.paste(Image.new("RGB", image.size, palette["water"]), mask=water)
...
class QuestService:
    ...
    async def _state_in_tx(self, db, user_id: int) -> dict:
        ...
        return {
            ...
            "map": {
                ...
                "mapgl_key": self.settings.mapgl_key,
    ...
    async def admin_overview(self) -> dict:
        ...
        return {
            ...
            "map": {"tile_url": self.settings.map_tile_url, "tile_urls": list(self.settings.map_tile_urls), "attribution": self.settings.map_attribution, "mapgl_key": self.settings.mapgl_key, "mapgl_style": self.settings.mapgl_style},
...
def create_web_app(service: QuestService, settings: Settings, bot: Bot, build_version: str) -> web.Application:
    ...
    async def security_middleware(request, handler):
        ...
        response.headers["Content-Security-Policy"] = (
            ...
            "script-src 'self' 'unsafe-inline' https://telegram.org https://mapgl.2gis.com; "
    ...
    async def health(_):
        ...
        return json_response({"service": "bibibike-quest", "build_version": build_version, "database": True,
                              ...
                              "tile_theme": settings.tile_theme, "tile_recolor_ready": imaging,
        ...
        for template in settings.tile_upstreams:
            ...
            if settings.tile_theme == "dark":
                try:
                    ...
                    # Перекраска не критична: лучше светлая карта, чем пустая.
                    log.warning("Не удалось перекрасить тайл %s", key)
            cache_put(tile_cache, key, data, 2200)
            return data
        return None
    ...
    def tile_numbers(lat: float, lon: float, zoom: int) -> tuple[int, int]:
        """Номер квадрата карты для координаты — стандартная схема OSM."""
        n = 2 ** zoom
    ...
    async def warm_tiles(_):
        """Заранее подготовить карту долины.

        Без прогрева первые движения по карте упираются в загрузку и
        перекраску каждого квадрата — отсюда рывки. Здесь всё, что нужно
        для Красной Поляны, готовится один раз при запуске и дальше
        отдаётся мгновенно из памяти. Работает в фоне: приложение
        доступно сразу, не дожидаясь прогрева.
        """
        if settings.tile_theme != "dark":
            return

        async def run():
            (south, west), (north, east) = QuestService.POLYANA_BOUNDS
            started = time.monotonic()
            done = 0
            # Масштабы обзора и среднего приближения: на них человек
            # проводит почти всё время. Крупные планы подгружаются по мере
            # надобности — качать их пачкой и невежливо к серверам OSM,
            # и бессмысленно: их тысячи.
            for zoom in (12, 13, 14, 15):
                x1, y1 = tile_numbers(north, west, zoom)
                x2, y2 = tile_numbers(south, east, zoom)
                for x in range(min(x1, x2), max(x1, x2) + 1):
                    for y in range(min(y1, y2), max(y1, y2) + 1):
                        ...
                        try:
                            ...
                            pass
                        # Пауза бережёт и наш сервер, и источник карты.
                        await asyncio.sleep(0.05)
            log.info("Карта долины прогрета: %s квадратов за %.0f с", done, time.monotonic() - started)
    ...
    app.router.add_get("/tiles/{style}/{z}/{x}/{y}.png", serve_tile)
    ...
    app.on_startup.append(warm_tiles)
