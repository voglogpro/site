...
TILE_BG = (18, 20, 23)    # фон готовой карты
TILE_INK = (186, 194, 205)  # предельный цвет дорог и подписей
TILE_INK_MAX = 150
TILE_GAIN = 6.0
TILE_WATER = (18, 34, 52)
TILE_PARK = (18, 32, 22)
...
def darken_tile(data: bytes) -> bytes:
    ...
    ink = deviation.point(lambda v: min(TILE_INK_MAX, int(v * TILE_GAIN)))
    out = ImageOps.colorize(ink, black=TILE_BG, white=TILE_INK)
    ...
    out.paste(Image.new("RGB", image.size, TILE_PARK), mask=park)
    out.paste(Image.new("RGB", image.size, TILE_WATER), mask=water)
