...
class QuestService:
    ...
    async def delete_participant(self, actor_id: int, session_id: str) -> None:
        ...
        async with self.db.transaction() as conn:
            ...
            await conn.execute(
                "INSERT INTO events(id,session_id,user_id,type,payload,created_at)"
                " VALUES(?,?,?,?,?,?)",
                (uuid.uuid4().hex, None, actor_id, "admin_deleted_participant",
                 json.dumps({"session_id": session_id, "user_id": row["user_id"]},
