...
class Database:
    ...
    async def initialize(self) -> None:
        ...
        for table in ("points", "session_points"):
            try:
                await self._db.execute(
                    f"ALTER TABLE {table} ADD COLUMN photo_url TEXT NOT NULL DEFAULT ''"
                )
            except Exception:
...
class QuestService:
    ...
    async def start(self, identity: TelegramIdentity, privacy_accepted: bool) -> dict:
        ...
        async with self.lock_for(identity.user_id):
            async with self.db.transaction() as db:
                ...
                for point in points:
                    await db.execute(
                        """INSERT INTO session_points(session_id,point_id,seq,point_name,address,latitude,longitude,radius_m,reward_title,reward_text,partner_hours,photo_url)
                           VALUES(?,?,?,?,?,?,?,?,?,?,?,?)""",
    ...
    async def _state_in_tx(self, db, user_id: int) -> dict:
        ...
        if session:
            ...
            points = await (await db.execute(
                "SELECT id point_id,seq,name point_name,address,latitude,longitude,radius_m,reward_title,reward_text,partner_hours,photo_url,NULL location_seen_at,NULL qr_seen_at,NULL completed_at,NULL reward_code FROM points WHERE campaign_id=? AND active=1 ORDER BY seq",
                (campaign["id"],),
            )).fetchall()
        last_age = None
    ...
    async def admin_update_point(self, admin_id: int, point_id: int, payload: dict) -> dict:
        ...
        async with self.db.transaction() as db:
            ...
            await db.execute(
                """UPDATE points SET name=?,address=?,latitude=?,longitude=?,radius_m=?,reward_title=?,reward_text=?,partner_hours=?,photo_url=?,updated_at=? WHERE id=?""",
            ...
            await db.execute(
                """UPDATE session_points
                   SET point_name=?,address=?,latitude=?,longitude=?,radius_m=?,
                       reward_title=?,reward_text=?,partner_hours=?,photo_url=?
    ...
    async def admin_set_point_photo(self, admin_id: int, point_id: int, relative_path: str) -> dict:
        """Фото прислано боту и уже сохранено на диске — привязываем к точке.
...
def build_router(service: QuestService, settings: Settings) -> Router:
    ...
    async def apply_point_photo(callback: CallbackQuery):
        ...
        try:
            point_id = int(choice)
            # Уникальное имя на каждую загрузку: старая ссылка на фото этой же
            # точки остаётся рабочей ровно до этого момента, новая сразу же не
            # конфликтует с кэшем браузера или Telegram — годичный Cache-Control
            # на /media безопасен именно поэтому.
            filename = f"point-{point_id}-{secrets.token_hex(6)}.jpg"
            media_dir = settings.data_dir / "media"
            media_dir.mkdir(parents=True, exist_ok=True)
            tg_file = await bot.get_file(file_id)
            await bot.download_file(tg_file.file_path, destination=media_dir / filename)
            result = await service.admin_set_point_photo(callback.from_user.id, point_id, filename)
