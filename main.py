        ...
        await self._db.execute(
            """UPDATE support_messages
               SET text=REPLACE(
                   ...
                   'Заявка на финальную награду: выдать только подписку на 30 дней. 300 Бибибонусов уже начислены автоматически за три этапа — повторно не начислять. '
               )
               WHERE kind='premium_request'
                 AND text LIKE 'Заявка на финальную награду: подписка 30 дней + 300 Бибибонусов.%'"""
        )
    ...
    async def admin_overview(self) -> dict:
        ...
        return {
            ...
            "map": {"attribution": self.settings.map_attribution, "mapgl_key": mapgl_key_for(self.settings), "mapgl_style": self.settings.mapgl_style, "mapgl_styles": {"light": self.settings.mapgl_style_light, "dark": self.settings.mapgl_style_dark}, # Подложка для карты в CRM. В приложении карту рисует 2ГИС, а панели
                    ...
                    "tile_url": self.settings.map_tile_url, "tile_urls": list(CRM_TILE_SOURCES)},
...
def create_web_app(service: QuestService, settings: Settings, bot: Bot, build_version: str) -> web.Application:
    ...
    async def quest_deep_link(raw: str) -> str:
        ...
        По умолчанию это прямая ссылка на бота в Telegram:
        https://t.me/<бот>?startapp=<код>.

        Раньше печатался собственный адрес вида <домен>/q/<код>, и это
        оказалось ошибкой: домен выдаётся хостингом и меняется при
        пересоздании бота — напечатанные таблички разом умерли. Имя бота
        принадлежит нам и переезд хостинга переживает, поэтому ссылка
        привязана к нему.

        Свой домен всё ещё можно поставить через PUBLIC_LINK_BASE, но
        только если это собственный домен компании (например bb.bike),
        а не временный адрес хостинга.
        """
        base = settings.public_link_base.strip()
        if base:
            return f"{base.rstrip('/')}/q/{raw}"
        link = await telegram_link(raw)
        if link:
            return link
        # Крайний случай: имя бота ещё не получено — тогда прежняя схема.
        fallback = settings.webapp_url.rstrip("/")
        return f"{fallback}/q/{raw}" if fallback else raw
